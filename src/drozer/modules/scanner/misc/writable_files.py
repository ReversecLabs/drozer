...
class WritableFiles(Module, common.ToyBox, common.Shell, common.SuperUser, common.PackageManager):

    name = "Find world-writable files in the given folder or package"
    description = "Find world-writable files in the given folder or package. Target can be a directory path or package name."
...
dz> run scanner.misc.writablefiles com.android.browser
Discovered world-writable files in /data/data/com.android.browser:
  ...
"""
    ...
    def execute(self, arguments):
        ...
        if self.isFindInstalled():
            command = "find %s \( -type b -o -type c -o -type f -o -type s \) -perm -o=w \-exec ls {} \;" % resolved_path
        else:
            if (self.isToyBoxInstalled() == True):
                command = self.toyboxPath() + " find %s \( -type b -o -type c -o -type f -o -type s \) -perm -o=w \-exec ls {} \;" % resolved_path
            else:
